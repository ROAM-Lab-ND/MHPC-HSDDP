classdef PlanarQuadruped < BaseDyn
    ...
    methods
        function dynInfo = dynamics_par(Quad,x,u,mode)
            q   = x(1:7, 1);
            qd  = x(8:end, 1);
           ...
           dynInfo.fx = eye(Quad.xsize) + fcon_x*Quad.dt;
           dynInfo.fu = fcon_u*Quad.dt;
           gx = zeros(Quad.ysize, Quad.xsize);
           gu = zeros(Quad.ysize, Quad.usize);           
        ...
        function Px = resetmap_par(Quad,x,mode,next_mode)
            qpre = x(1:7, 1);
            qdpre = x(8:end, 1);
