...
for idx = problem_data.n_Phases
    HybridTrajectory(idx).Initialization(Phases(idx).model.xsize,...
...
HybridTrajectory(1).set_initial_condition(x0);

HSDDP = HybridSystemsDDP(Phases, HybridTrajectory);

HSDDP.set_initial_condition(x0);

% Initialize trajectory using a heuristic controller
% heuristicControl(HybridTrajectory);

Ustart = cell(1,length(HybridTrajectory));

for idx = 1:length(HybridTrajectory)
    Ustart{idx} = HybridTrajectory(idx).Ubar;
end

HSDDP.set_initial_guess(Ustart);

[xopt, uopt, K] = HSDDP.Run(options);
